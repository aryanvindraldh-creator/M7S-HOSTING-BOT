...
# ----- NEW: Webhook endpoint for payment callbacks -----
@app.route('/webhook/zapupi', methods=['POST'])
def zapupi_webhook():
    """Handle Zapupi payment callback"""
...
def binance_webhook():
    """Handle Binance payment callback"""
    data = request.get_json()
    logger.info(f"Binance webhook received: {data}")
    # Verify signature (Binance uses HMAC)
...
# --- NEW: Data structures for paid system ---
plans_cache = {}                # plan_id -> plan dict
zapupi_settings = {}            # will hold api_key, enabled
binance_settings = {}           # will hold api_key, secret_key, enabled
pending_payments = {}           # user_id -> {payment_id, plan_id, gateway, ...}
transaction_cache = {}          # for quick lookup
...
# --- Command Button Layouts (updated with new menu items) ---
...
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    # NEW: Payment & Plan Management
    ["💲 Payment Settings", "📋 Plan Management"],
    ["📋 View Plans", "🛒 Purchase Plan"]
]

# --- Database Setup (modified to add new tables) ---
def init_db():
    ...
    try:
        ...
        # Zapupi settings
        c.execute('''CREATE TABLE IF NOT EXISTS zapupi_settings (
                     id INTEGER PRIMARY KEY AUTOINCREMENT,
                     api_key TEXT,
        ...
        # Binance settings
        c.execute('''CREATE TABLE IF NOT EXISTS binance_settings (
                     id INTEGER PRIMARY KEY AUTOINCREMENT,
                     api_key TEXT,
                     secret_key TEXT,
                     gateway_enabled INTEGER DEFAULT 0,
        ...
        # User active plans (subscription via plan)
        ...
        if c.fetchone()[0] == 0:
            ...
            for name, price, duration, limit, desc in default_plans:
                c.execute('''INSERT INTO plans (name, price, duration_days, bot_limit, description, status)
                             VALUES (?, ?, ?, ?, ?, 'active')''',
                          (name, price, duration, limit, desc))
...
def load_data():
    """Load data from database into memory (existing + new)"""
    ...
    try:
        ...
        conn.close()
        logger.info(f"Data loaded: {len(active_users)} users, {len(plans_cache)} plans, "
                    f"Zapupi enabled: {zapupi_settings.get('enabled', False)}, Binance enabled: {binance_settings.get('enabled', False)}")
    except Exception as e:
...
# --- Modified Helper Functions (with plan check) ---
...
# NEW: get user's active plan
def get_user_active_plan(user_id):
    """Return the active plan for a user, or None if no active plan."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('''SELECT plan_id, bot_limit, expiry_date FROM user_plans
...
def get_user_plan_limit(user_id):
    """Return the bot limit from the user's active plan, or None if no active plan."""
    plan = get_user_active_plan(user_id)
    if plan:
        return plan['bot_limit']
    return None

# Override the original get_user_file_limit to consider paid plans
def get_user_file_limit(user_id):
    """Get the file upload limit for a user (modified to check active plan first)"""
...
# --- Map Telegram import names to actual PyPI package names (unchanged) ---
TELEGRAM_MODULES = {
    # Main Bot Frameworks
...
# --- Manual Modules Installation System (unchanged) ---
...
# --- NEW: Plan Management Menus ---
def create_plan_management_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton('➕ Create Plan', callback_data='admin_create_plan'),
        types.InlineKeyboardButton('📋 View Plans', callback_data='admin_view_plans')
    )
    markup.row(
        types.InlineKeyboardButton('✏️ Edit Plan', callback_data='admin_edit_plan'),
...
def create_payment_settings_menu():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        types.InlineKeyboardButton('💲 Zapupi Settings', callback_data='admin_zapupi_settings'),
...
# --- Automatic Package Installation & Script Running (unchanged) ---
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        active_plan = get_user_active_plan(user_id)
        if active_plan:
            user_level += f" (Plan {active_plan['plan_id']})"
...
def _logic_statistics(message):
    ...
    if user_id in admin_ids:
        stats_msg_admin = (f"🔒 Bot Status: {'🔴 Locked' if bot_locked else '🟢 Unlocked'}\n"
                           ...
                           f"💳 Transactions: {len(transaction_cache) if hasattr(transaction_cache, '__len__') else 'N/A'}\n"
...
# --- New Admin Functions for Channel Management (unchanged) ---
...
# --- NEW: commands for plans and purchases ---
...
# --- Document (File) Handler (unchanged) ---
...
def handle_callbacks(call):
    ...
    try:
        ...
        # --- Mandatory Channels Callbacks (existing) ---
        ...
        # --- Security Approval Callbacks (existing) ---
        ...
        elif data.startswith('buy_plan_'): callback_buy_plan(call)
        elif data.startswith('pay_zapupi_'): callback_pay_zapupi(call)
        elif data.startswith('pay_binance_'): callback_pay_binance(call)
        elif data.startswith('verify_zapupi_'): callback_verify_zapupi(call)
        elif data.startswith('verify_binance_'): callback_verify_binance(call)
        elif data == 'cancel_purchase': callback_cancel_purchase(call)

        elif data == 'admin_create_plan': admin_required_callback(call, callback_admin_create_plan)
        elif data == 'admin_view_plans': admin_required_callback(call, callback_admin_view_plans)
        elif data == 'admin_edit_plan': admin_required_callback(call, callback_admin_edit_plan)
        elif data == 'admin_delete_plan': admin_required_callback(call, callback_admin_delete_plan)

        elif data == 'admin_zapupi_settings': admin_required_callback(call, callback_admin_zapupi_settings)
        elif data == 'admin_binance_settings': admin_required_callback(call, callback_admin_binance_settings)
        elif data == 'admin_transactions': admin_required_callback(call, callback_admin_transactions)
        elif data == 'admin_revenue': admin_required_callback(call, callback_admin_revenue)

        elif data == 'zapupi_set_key': admin_required_callback(call, callback_zapupi_set_key)
        elif data == 'zapupi_toggle': admin_required_callback(call, callback_zapupi_toggle)
        elif data == 'zapupi_delete': admin_required_callback(call, callback_zapupi_delete)

        elif data == 'binance_set_keys': admin_required_callback(call, callback_binance_set_keys)
        elif data == 'binance_toggle': admin_required_callback(call, callback_binance_toggle)
        elif data == 'binance_delete': admin_required_callback(call, callback_binance_delete)
...
# --- User Callbacks (existing, unchanged) ---
...
def back_to_main_callback(call):
    ...
    if active_plan:
        ...
        if user_id == OWNER_ID: user_status = "👑 Owner"
            ...
            expiry_date = user_subscriptions[user_id].get('expiry')
            if expiry_date and expiry_date > datetime.now():
                user_status = "⭐ Premium (Legacy)"; days_left = (expiry_date - datetime.now()).days
                expiry_info = f"\n⏳ Subscription expires in: {days_left} days"
...
# --- Admin Callback Implementations (existing, unchanged) ---
...
def display_users_list(chat_id, message_id, users_list, page, total_pages, chunk_size):
    ...
    for i, user_id in enumerate(user_chunk, start=start_idx + 1):
        ...
        elif get_user_active_plan(user_id): status = "⭐"
...
# --- Admin Settings Callbacks (existing, unchanged) ---
...
def show_plans_to_user(chat_id, user_id):
    """Display available plans with purchase buttons."""
...
def initiate_purchase(user_id, plan_id, chat_id):
    """Start purchase flow: ask for payment gateway choice."""
...
def handle_payment_gateway(user_id, plan_id, gateway, chat_id):
    """Create payment and send invoice link."""
    plan = plans_cache.get(int(plan_id))
    if not plan:
...
def verify_payment_manually(user_id, transaction_id, gateway, chat_id):
    """Manual verification by user clicking 'I've Paid'."""
    if gateway == 'zapupi':
...
# --- Callbacks for plans and payments ---
...
# --- Admin payment settings callbacks ---
def callback_admin_zapupi_settings(call):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("➕ Set API Key", callback_data="zapupi_set_key"),
...
def callback_admin_binance_settings(call):
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("➕ Set Keys", callback_data="binance_set_keys"),
...
def verify_binance_payment(order_id):
    """Verify Binance Pay order status."""
    api_key = binance_settings.get('api_key')
    secret_key = binance_settings.get('secret_key')
    if not api_key or not secret_key:
...
def process_zapupi_webhook(data):
    """Handle Zapupi webhook (run in thread)"""
    logger.info(f"Processing Zapupi webhook: {data}")
    invoice_id = data.get('invoice_id')
    status = data.get('status')
    if status != 'paid':
        return
...
def process_binance_webhook(data):
    ...
    logger.info(f"Processing Binance webhook: {data}")
    order_id = data.get('orderId')
    status = data.get('status')
    if status != 'PAID':
        return
...
def activate_user_plan(user_id, plan_id, transaction_id):
    """Activate a plan for a user based on a successful transaction."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
