...
@app.route('/')
def home():
    return "MASTER HOSTING BOT - Advanced Bot Hosting Platform"
...
YOUR_USERNAME = os.getenv('YOUR_USERNAME', '@MASTER_BOT')  # Changeable
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL', '@MASTER_UPDATES')
SUPPORT_USERNAME = os.getenv('SUPPORT_USERNAME', '@MASTER_SUPPORT')

# Free users get 0 limit unless they are in trial or have paid plan
FREE_USER_LIMIT = 0
...
def init_db():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS plans (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, price_inr REAL NOT NULL, duration_days INTEGER NOT NULL, bot_limit INTEGER NOT NULL, ram_limit INTEGER DEFAULT 128, description TEXT, status TEXT DEFAULT 'active', created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP)''')
        ...
        # transactions table now includes 'type' column
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS user_plans (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, plan_id INTEGER, start_date TEXT, expiry_date TEXT, bot_limit INTEGER, ram_limit INTEGER DEFAULT 128, active INTEGER DEFAULT 1, transaction_id INTEGER, created_at TEXT DEFAULT CURRENT_TIMESTAMP, updated_at TEXT DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES active_users(user_id), FOREIGN KEY (plan_id) REFERENCES plans(id))''')
        ...
        # Trial table with last_trial_date
        ...
        # --- Migrate existing transactions table to add 'type' column if missing ---
        try:
        ...
        # Insert default plans with RAM
        c.execute('SELECT COUNT(*) FROM plans')
        if c.fetchone()[0] == 0:
            default_plans = [
                ('Starter Lite', 49, 7, 1, 128, '1 bot, 128 MiB RAM, 7 days'),
                ('Starter', 499, 30, 2, 256, '2 bots, 256 MiB RAM, 30 days'),
                ('Pro', 1299, 90, 5, 512, '5 bots, 512 MiB RAM, 90 days'),
                ('Enterprise', 2499, 365, 20, 2048, '20 bots, 2 GiB RAM, 365 days')
            ]
            for name, price, duration, limit, ram, desc in default_plans:
                c.execute('''INSERT INTO plans (name, price_inr, duration_days, bot_limit, ram_limit, description, status) VALUES (?, ?, ?, ?, ?, ?, 'active')''', (name, price, duration, limit, ram, desc))
            logger.info("Default plans inserted with RAM limits.")
...
def load_data():
    ...
    try:
        ...
        # Load plans with ram_limit
        c.execute('SELECT id, name, price_inr, duration_days, bot_limit, ram_limit, description, status FROM plans')
        for row in c.fetchall():
            plans_cache[row[0]] = {'id': row[0], 'name': row[1], 'price_inr': row[2], 'duration_days': row[3], 'bot_limit': row[4], 'ram_limit': row[5], 'description': row[6], 'status': row[7]}
        # Load zapupi, binance settings
        ...
        conn.close()
        logger.info(f"Data loaded: {len(active_users)} users, {len(plans_cache)} plans, wallets: {len(user_wallets)}")
    except Exception as e:
...
# --- define list_plans (was missing) ---
def list_plans(active_only=True):
    """Return list of plans from cache."""
    if active_only:
...
def check_auto_renewals():
    while True:
        try:
            ...
            c = conn.cursor()
            now = datetime.now().isoformat()
            c.execute('''SELECT up.id, up.user_id, up.plan_id, up.bot_limit, up.ram_limit, p.price_inr, p.duration_days, p.name
                         FROM user_plans up
                         JOIN plans p ON up.plan_id = p.id
                         WHERE up.active = 1 AND up.expiry_date < datetime('now')''')
            expired_plans = c.fetchall()
            conn.close()
            for plan_id, user_id, plan_db_id, bot_limit, ram_limit, price, duration_days, plan_name in expired_plans:
                balance = get_wallet_balance(user_id)
                if balance >= price:
                    success, msg = deduct_wallet(user_id, price, f"Auto-renewal of plan {plan_name} (ID: {plan_db_id})")
                    if success:
                        ...
                        logger.info(f"Auto-renewed plan for user {user_id}, plan {plan_db_id}, new expiry {new_expiry}")
                        try:
                            bot.send_message(user_id, f"🔄 Your plan '{plan_name}' has been auto-renewed for another {duration_days} days. Amount ₹{price:.2f} deducted from wallet. New expiry: {new_expiry.strftime('%Y-%m-%d %H:%M:%S')}")
                        except:
                            pass
                    else:
                    ...
                    try:
                        bot.send_message(user_id, f"❌ Your plan '{plan_name}' has expired and could not be auto-renewed due to insufficient wallet balance. Please top up your wallet or purchase a new plan.")
                    except:
                        pass
        except Exception as e:
...
def create_transaction(user_id, amount, gateway, transaction_id, plan_id=None, txn_type='plan'):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('''INSERT INTO transactions (user_id, plan_id, amount, currency, gateway, transaction_id, status, type, created_at)
                 VALUES (?, ?, ?, 'INR', ?, ?, 'pending', ?, datetime("now"))''',
              (user_id, plan_id, amount, gateway, transaction_id, txn_type))
...
def complete_transaction(transaction_id, user_id, plan_id=None, amount=None, txn_type='plan'):
    if txn_type == 'plan' and plan_id:
        success, result = activate_user_plan(user_id, plan_id, transaction_id)
        if success:
            expiry_date = result
...
# --- Zapupi integration ---
def create_zapupi_order(user_id, amount, order_id, plan_id=None, txn_type='plan'):
...
def create_binance_order(user_id, amount, order_id, plan_id=None, txn_type='plan'):
    ...
    url = "https://api.binance.com/sapi/v1/pay/order"
    ...
    params = {
        ...
        "currency": "USDT",
        "goodsName": f"{txn_type}_{plan_id}" if plan_id else txn_type,
        "goodsDetail": f"{txn_type} for user {user_id}",
    ...
    try:
        ...
        payment_url = data.get('payment_url')
        return order_id_resp, payment_url
    except Exception as e:
        logger.error(f"Binance order creation error: {e}")
        return None, str(e)

def verify_binance_order(order_id):
    api_key = binance_settings.get('api_key')
    secret_key = binance_settings.get('secret_key')
    if not api_key or not secret_key:
...
def get_user_active_plan(user_id):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('''SELECT plan_id, bot_limit, ram_limit, expiry_date FROM user_plans WHERE user_id = ? AND active = 1 AND expiry_date > datetime('now') ORDER BY expiry_date DESC LIMIT 1''', (user_id,))
    row = c.fetchone()
    conn.close()
    if row:
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('💰 Wallet Management', callback_data='admin_wallet_management')
        ]
        ...
        markup.add(types.InlineKeyboardButton('💰 Wallet', callback_data='wallet_balance'))
...
def create_add_balance_menu():
    ...
    markup.row(types.InlineKeyboardButton('💲 Pay via Zapupi', callback_data='add_balance_zapupi'),
               types.InlineKeyboardButton('🟣 Pay via Binance', callback_data='add_balance_binance'))
...
def _logic_send_welcome(message):
    ...
    welcome_msg_text = (f"〽️ Welcome, {user_name}!\n\n🆔 Your User ID: `{user_id}`\n"
                        ...
                        f"💵 To add balance, use /addbalance or the button below.\n\n"
...
def _logic_bot_speed(message):
    ...
    try:
        ...
        speed_msg = (f"⚡ Bot Speed & Status:\n\n⏱️ API Response Time: {response_time} ms\n"
                     ...
                     f"💰 Wallet: ₹{get_wallet_balance(user_id):.2f}")
...
• /addbalance - Add balance to your wallet (via Zapupi/Binance)
...
**💰 Wallet System:**
• Check balance with /balance or button
• Add balance with /addbalance (minimum ₹10)
...
• Create, edit, delete subscription plans (with RAM limits)
...
# --- Wallet Management Admin Commands ---
...
def process_add_balance_amount(message):
    ...
    try:
        ...
        if amount < 10:
            ...
            return
        if not hasattr(bot, 'pending_add_balance'):
            bot.pending_add_balance = {}
        bot.pending_add_balance[user_id] = amount
...
def initiate_add_balance_payment(user_id, amount, gateway, chat_id):
    ...
    if gateway == 'zapupi':
        ...
        txn_id, payment_url = create_binance_order(user_id, amount, order_id, txn_type='balance')
        if not txn_id:
            bot.send_message(chat_id, f"❌ Failed to create Binance order: {payment_url}")
            return
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("💳 Pay Now", url=payment_url))
        markup.add(types.InlineKeyboardButton("✅ I've Paid (Verify)", callback_data=f"verify_balance_binance_{order_id}"))
        bot.send_message(chat_id, f"🔗 Pay via Binance: {payment_url}\nOrder ID: `{order_id}`\nAfter payment, click 'I've Paid'.", reply_markup=markup, parse_mode='Markdown')

# --- Admin plan & payment management functions ---
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "💰 Wallet Balance": _logic_wallet_balance,
    "💰 Wallet Management": _logic_wallet_management,
    "💳 Add Balance": _logic_add_balance
}
...
def handle_callbacks(call):
    ...
    if is_user_banned(user_id) and data not in ['back_to_main']:
        ...
        return
    if data not in ['check_subscription_status', 'back_to_main', 'manual_install', 'wallet_balance', 'add_balance']:
        is_subscribed, not_joined = check_mandatory_subscription(user_id)
        if not is_subscribed and user_id not in admin_ids:
            subscription_message, markup = create_subscription_check_message(not_joined)
            ...
            return
    if bot_locked and user_id not in admin_ids and data not in ['back_to_main', 'speed', 'stats', 'check_subscription_status', 'manual_install', 'wallet_balance', 'add_balance']:
    ...
    try:
        ...
        elif data == 'add_balance_binance':
            callback_add_balance_binance(call)
        elif data.startswith('verify_balance_zapupi_'):
            callback_verify_balance_zapupi(call)
        elif data.startswith('verify_balance_binance_'):
            callback_verify_balance_binance(call)
        else:
...
def process_user_info(message):
    ...
    try:
        ...
        active_plan = get_user_active_plan(user_id)
        if active_plan:
            info_parts.append(f"⭐ **Plan:** {active_plan['plan_id']} (Bot limit: {active_plan['bot_limit']}, RAM: {active_plan['ram_limit']} MiB)")
        ...
        wallet_balance = get_wallet_balance(user_id)
        info_parts.append(f"💰 **Wallet Balance:** ₹{wallet_balance:.2f}")
...
def system_info_callback(call):
    ...
    try:
        ...
        info_text = "\n".join(info_parts)
        # Use plain text to avoid markdown errors
...
def handle_payment_gateway(user_id, plan_id, gateway, chat_id):
    ...
    order_id = generate_transaction_id()
    txn_db_id = create_transaction(user_id, amount, gateway, order_id, plan_id=plan_id, txn_type='plan')
    if gateway == 'zapupi':
        txn_id, payment_url = create_zapupi_order(user_id, amount, order_id, plan_id=plan_id, txn_type='plan')
        if not txn_id:
            bot.send_message(chat_id, f"❌ Failed to create Zapupi order: {payment_url}")
            return
        markup = types.InlineKeyboardMarkup()
        ...
        txn_id, payment_url = create_binance_order(user_id, amount, order_id, plan_id=plan_id, txn_type='plan')
        if not txn_id:
            bot.send_message(chat_id, f"❌ Failed to create Binance order: {payment_url}")
            return
        markup = types.InlineKeyboardMarkup()
...
def verify_payment_manually(user_id, transaction_id, gateway, chat_id):
    ...
    if gateway == 'zapupi':
        ...
        success, msg = verify_binance_order(transaction_id)
    if success:
        update_transaction_status(txn_db_id, 'completed')
        success2, result = complete_transaction(transaction_id, user_id_db, plan_id, amount, txn_type)
        if success2:
...
def callback_admin_create_plan(call):
    if call.from_user.id not in admin_ids:
        ...
        return
    msg = bot.send_message(call.message.chat.id, "📝 **Create New Plan**\n\nSend plan details in format:\n`name | price_inr | duration_days | bot_limit | ram_limit | description`\nExample: `Premium | 999 | 30 | 5 | 512 | Premium plan with 5 bots and 512 MiB RAM`\n/cancel to cancel.")
...
def process_create_plan(message):
    ...
    parts = message.text.split('|')
    if len(parts) != 6:
        bot.reply_to(message, "❌ Invalid format. Please use: `name | price_inr | duration_days | bot_limit | ram_limit | description`")
        return
    name = parts[0].strip()
    try:
        price_inr = float(parts[1].strip())
        duration = int(parts[2].strip())
        bot_limit = int(parts[3].strip())
        ram_limit = int(parts[4].strip())
    except ValueError:
        bot.reply_to(message, "❌ Price, duration, bot limit, and RAM limit must be numeric.")
        return
    description = parts[5].strip()
    plan_id = create_plan(name, price_inr, duration, bot_limit, ram_limit, description)
...
def callback_admin_view_plans(call):
    ...
    text = "📋 **All Plans**\n\n"
    for p in plans:
        price_display = get_price_display(p['price_inr'])
        text += f"ID: {p['id']}\nName: {p['name']}\nPrice: {price_display}\nDuration: {p['duration_days']} days\nBot Limit: {p['bot_limit']}\nRAM Limit: {p['ram_limit']} MiB\nStatus: {p['status']}\nDesc: {p['description']}\n\n"
...
def callback_admin_edit_plan(call):
    if call.from_user.id not in admin_ids:
        bot.answer_callback_query(call.id, "Admin only.")
        return
    msg = bot.send_message(call.message.chat.id, "✏️ **Edit Plan**\n\nSend plan ID and fields to update in format:\n`plan_id | field=value | field2=value2`\n\nAvailable fields: name, price_inr, duration_days, bot_limit, ram_limit, description, status (active/inactive)\nExample: `3 | price_inr=1200 | ram_limit=1024`\n/cancel to cancel.")
...
def process_edit_plan(message):
    ...
    for part in parts[1:]:
        ...
        if key in ['name', 'description', 'status']:
            updates[key] = value
        elif key in ['price_inr', 'duration_days', 'bot_limit', 'ram_limit']:
            try:
                updates[key] = float(value) if key == 'price_inr' else int(value)
            except ValueError:
...
def process_admin_add_balance(message):
    ...
    try:
        ...
        try:
            bot.send_message(user_id, f"💰 Admin added ₹{amount:.2f} to your wallet. New balance: ₹{get_wallet_balance(user_id):.2f}")
        except:
            pass
    except Exception as e:
        bot.reply_to(message, f"❌ Error: {str(e)}")
...
def process_admin_deduct_balance(message):
    ...
    try:
        ...
        if success:
            ...
            try:
                bot.send_message(user_id, f"💰 Admin deducted ₹{amount:.2f} from your wallet. New balance: ₹{get_wallet_balance(user_id):.2f}")
            except:
                pass
        else:
...
def callback_add_balance_zapupi(call):
    user_id = call.from_user.id
    if not hasattr(bot, 'pending_add_balance') or user_id not in bot.pending_add_balance:
        bot.answer_callback_query(call.id, "⚠️ No pending amount. Start again with /addbalance.", show_alert=True)
        return
    amount = bot.pending_add_balance.pop(user_id)
    initiate_add_balance_payment(user_id, amount, 'zapupi', call.message.chat.id)
    bot.answer_callback_query(call.id)

def callback_add_balance_binance(call):
    user_id = call.from_user.id
    if not hasattr(bot, 'pending_add_balance') or user_id not in bot.pending_add_balance:
        bot.answer_callback_query(call.id, "⚠️ No pending amount. Start again with /addbalance.", show_alert=True)
        return
    amount = bot.pending_add_balance.pop(user_id)
    initiate_add_balance_payment(user_id, amount, 'binance', call.message.chat.id)
    bot.answer_callback_query(call.id)
...
def callback_verify_balance_binance(call):
    user_id = call.from_user.id
    order_id = call.data.split('_')[3]
    verify_payment_manually(user_id, order_id, 'binance', call.message.chat.id)
    bot.answer_callback_query(call.id)

# --- Payment gateway core functions (already defined) ---

# --- Auto-verification background task ---
        ...
        try:
            ...
            for txn_id, user_id, plan_id, amount, transaction_id, gateway, txn_type in pending:
                ...
                if success:
                    ...
                    success2, result = complete_transaction(transaction_id, user_id, plan_id, amount, txn_type)
                    if success2:
...
def process_zapupi_webhook(data):
    ...
    success, result = complete_transaction(order_id, user_id, plan_id, amount, txn_type)
    if success:
...
def process_binance_webhook(data):
    ...
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT id, user_id, plan_id, amount, type FROM transactions WHERE transaction_id = ? AND gateway = "binance"', (order_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        logger.warning(f"Transaction not found for order_id {order_id}")
...
def create_plan(name, price_inr, duration_days, bot_limit, ram_limit, description):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('''INSERT INTO plans (name, price_inr, duration_days, bot_limit, ram_limit, description, status)
                 VALUES (?, ?, ?, ?, ?, ?, 'active')''',
              (name, price_inr, duration_days, bot_limit, ram_limit, description))
    plan_id = c.lastrowid
    conn.commit()
...
def update_plan(plan_id, name=None, price_inr=None, duration_days=None, bot_limit=None, ram_limit=None, description=None, status=None):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    updates = []
    params = []
    ...
    if price_inr is not None:
        updates.append("price_inr = ?")
        params.append(price_inr)
    ...
    if ram_limit is not None:
        updates.append("ram_limit = ?")
        params.append(ram_limit)
...
def activate_user_plan(user_id, plan_id, transaction_id):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT duration_days, bot_limit, ram_limit FROM plans WHERE id = ?', (plan_id,))
    row = c.fetchone()
    if not row:
        conn.close()
        return False, "Plan not found"
    duration_days, bot_limit, ram_limit = row
    ...
    c.execute('''INSERT INTO user_plans (user_id, plan_id, start_date, expiry_date, bot_limit, ram_limit, active, transaction_id)
                 VALUES (?, ?, ?, ?, ?, ?, 1, ?)''',
              (user_id, plan_id, start_date.isoformat(), expiry_date.isoformat(), bot_limit, ram_limit, transaction_id))
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    logger.info("="*50 + "\n🤖 MASTER HOSTING BOT Starting Up... (All Features Fixed)\n" + "="*50)
    keep_alive()
