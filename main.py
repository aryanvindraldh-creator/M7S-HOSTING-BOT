...
# Exchange rate (INR to USD) – can be updated via admin later
...
def handle_payment_gateway(user_id, plan_id, gateway, chat_id):
    ...
    if gateway == 'zapupi':
        ...
        order_id, payment_url = create_binance_payment(user_id, plan_id, amount, currency='INR')  # Binance supports INR?
        # For Binance, we might need to convert to USDT. For simplicity, we'll keep INR and let Binance handle conversion.
        if not order_id:
...
# --- Zapupi Settings callbacks (with null check fix) ---
...
# --- Binance Settings callbacks (with null check fix) ---
...
# --- Payment gateway core functions (updated for Zapupi) ---
def create_zapupi_invoice(user_id, plan_id, amount, currency='INR'):
    ...
    payload = {
        ...
        "order_id": order_id,
        "amount": str(amount),
        "customer_mobile": "9999999999",  # Placeholder – you can ask user for phone later
    ...
    try:
        ...
        if data.get('status') == 'success':
            invoice_id = data.get('txn_id')  # or order_id
...
def verify_zapupi_payment(invoice_id):
    # Zapupi may have a verification endpoint; we rely on webhook.
    # For manual verification, we can check order status via API if available.
    # For now, we assume webhook will update transaction status.
    # We'll just query our local DB to see if transaction is marked completed.
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    ...
    if row and row[0] == 'completed':
        ...
        # Optionally call Zapupi status API if available
...
def create_binance_payment(user_id, plan_id, amount, currency='INR'):
    ...
    # Binance Pay expects amount in crypto; we'll convert INR to USDT using exchange rate
    # For simplicity, we assume user will pay in USDT; we'll show amount in USDT.
...
def process_zapupi_webhook(data):
    # Expected data: { "order_id": "...", "txn_id": "...", "status": "success", ... }
