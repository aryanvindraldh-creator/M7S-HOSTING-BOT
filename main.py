...
# --- Configuration ---
TOKEN = os.getenv('TOKEN', '8485769270:AAFBL7Y2ONjs1qJdp1SXiPPAEOnJBesI_aw')
...
COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["📅 My Subscription"],
    ["🎁 Get Trial"]   # New button for trial
]
...
def init_db():
    ...
    try:
        ...
        # NEW: Trial table
        c.execute('''CREATE TABLE IF NOT EXISTS user_trials (user_id INTEGER PRIMARY KEY, start_time TEXT, expiry_time TEXT, active INTEGER DEFAULT 1)''')

        c.execute('SELECT COUNT(*) FROM plans')
        if c.fetchone()[0] == 0:
            default_plans = [
                ('Starter', 499, 30, 2, 'Basic plan: 2 bots, 30 days'),
                ('Pro', 1299, 90, 5, 'Pro plan: 5 bots, 90 days'),
                ('Enterprise', 2499, 365, 20, 'Enterprise: 20 bots, 365 days')
            ]
            for name, price, duration, limit, desc in default_plans:
                c.execute('''INSERT INTO plans (name, price_inr, duration_days, bot_limit, description, status) VALUES (?, ?, ?, ?, ?, 'active')''', (name, price, duration, limit, desc))
            logger.info("Default plans inserted (INR).")
...
def load_data():
    ...
    try:
        ...
        # Load new
        c.execute('SELECT id, name, price_inr, duration_days, bot_limit, description, status FROM plans')
        for row in c.fetchall():
            plans_cache[row[0]] = {'id': row[0], 'name': row[1], 'price_inr': row[2], 'duration_days': row[3], 'bot_limit': row[4], 'description': row[5], 'status': row[6]}
...
def get_user_trial(user_id):
    """Return trial expiry datetime if active, else None."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT expiry_time FROM user_trials WHERE user_id = ? AND active = 1', (user_id,))
    row = c.fetchone()
    conn.close()
    if row:
        try:
...
def activate_trial(user_id):
    """Activate a 1-hour trial for a user."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    ...
    c.execute('INSERT INTO user_trials (user_id, start_time, expiry_time, active) VALUES (?, ?, ?, 1)',
              (user_id, start.isoformat(), expiry.isoformat()))
    conn.commit()
    conn.close()
    logger.info(f"Trial activated for user {user_id}, expires at {expiry}")

def deactivate_trial(user_id):
    """Deactivate trial and delete user's files."""
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
...
def delete_user_files(user_id):
    """Delete all files (scripts and logs) for a user from disk and DB."""
...
def trial_expiry_checker():
    """Background thread to check and expire trials."""
    while True:
        try:
...
def get_user_file_limit(user_id):
    ...
    return FREE_USER_LIMIT  # 0 for free users
...
# --- Core logic functions (with expiry_info fix) ---
...
• /trial - Activate 1-hour free trial (1 bot)
...
def handle_payment_gateway(user_id, plan_id, gateway, chat_id):
    ...
    if gateway == 'zapupi':
        ...
        invoice_id, payment_url = create_zapupi_invoice(user_id, plan_id, amount, session)
        if not invoice_id:
...
def callback_admin_create_plan(call):
    if call.from_user.id not in admin_ids:
        ...
        return
    msg = bot.send_message(call.message.chat.id, "📝 **Create New Plan**\n\nSend plan details in format:\n`name | price_inr | duration_days | bot_limit | description`\nExample: `Premium | 999 | 30 | 5 | Premium plan with 5 bots`\n/cancel to cancel.")
...
def process_create_plan(message):
    ...
    parts = message.text.split('|')
    if len(parts) != 5:
        bot.reply_to(message, "❌ Invalid format. Please use: `name | price_inr | duration_days | bot_limit | description`")
        return
    name = parts[0].strip()
    try:
    ...
    plan_id = create_plan(name, price_inr, duration, bot_limit, description)
...
def callback_admin_edit_plan(call):
    if call.from_user.id not in admin_ids:
        bot.answer_callback_query(call.id, "Admin only.")
        return
    msg = bot.send_message(call.message.chat.id, "✏️ **Edit Plan**\n\nSend plan ID and fields to update in format:\n`plan_id | field=value | field2=value2`\n\nAvailable fields: name, price_inr, duration_days, bot_limit, description, status (active/inactive)\nExample: `3 | price_inr=1200 | status=inactive`\n/cancel to cancel.")
...
def process_edit_plan(message):
    ...
    for part in parts[1:]:
        ...
        if key in ['name', 'description', 'status']:
            updates[key] = value
        elif key in ['price_inr', 'duration_days', 'bot_limit']:
            try:
...
def create_zapupi_invoice(user_id, plan_id, amount, session=None):
    api_key = zapupi_settings.get('api_key')
    if not api_key:
        return None, "Zapupi API key not configured."
    order_id = f"ORD_{user_id}_{plan_id}_{int(time.time())}"
    ...
    payload = {
        ...
        "remark": f"Plan {plan_id} for user {user_id}",
    ...
    try:
        ...
        if data.get('status') == 'success':
            invoice_id = data.get('txn_id') or data.get('order_id')
...
def verify_zapupi_payment(invoice_id):
    # We rely on webhook; check local DB status
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT status FROM transactions WHERE transaction_id = ? AND gateway = "zapupi"', (invoice_id,))
    row = c.fetchone()
    conn.close()
    if row and row[0] == 'completed':
        return True, "Paid"
    else:
        return False, "Not paid yet. Please wait for webhook or try again later."

def create_binance_payment(user_id, plan_id, amount, currency='INR'):
...
def process_zapupi_webhook(data):
    txn_id = data.get('txn_id') or data.get('order_id')
    status = data.get('status')
    if status != 'success':
        return
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('SELECT id, user_id, plan_id FROM transactions WHERE transaction_id = ? AND gateway = "zapupi"', (txn_id,))
    row = c.fetchone()
    conn.close()
    if not row:
        logger.warning(f"Transaction not found for txn_id {txn_id}")
        return
...
def create_plan(name, price_inr, duration_days, bot_limit, description):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    c.execute('''INSERT INTO plans (name, price_inr, duration_days, bot_limit, description, status)
                 VALUES (?, ?, ?, ?, ?, 'active')''',
              (name, price_inr, duration_days, bot_limit, description))
    plan_id = c.lastrowid
    conn.commit()
...
def update_plan(plan_id, name=None, price_inr=None, duration_days=None, bot_limit=None, description=None, status=None):
    conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
    c = conn.cursor()
    updates = []
    params = []
...
if __name__ == '__main__':
    ...
    logger.info("="*50 + "\n🤖 DIV Hosting Bot Starting Up... (Paid + Trial)\n" + "="*50)
    keep_alive()
